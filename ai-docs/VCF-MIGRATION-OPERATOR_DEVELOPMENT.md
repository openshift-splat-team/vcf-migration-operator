# vcf-migration-operator Development Guide
...
This guide covers the development workflow for vcf-migration-operator.
...
```bash
git clone https://github.com/openshift-splat-team/vcf-migration-operator.git
cd vcf-migration-operator
```
...
# Or directly
go build -o bin/vcf-migration-operator ./cmd/...
```
...
# Or with podman
podman build -t vcf-migration-operator:latest .
```
...
# Run
./bin/vcf-migration-operator --help
```
...
# Or via command line
./bin/vcf-migration-operator --log-level=debug
```
...
# Debug
dlv debug ./cmd/vcf-migration-operator
```
...
- [Testing Guide](vcf-migration-operator_TESTING.md) - Test suites and strategies
